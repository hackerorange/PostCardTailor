$(function () {
    var postCardInfo = {
        ...
        submitMyPostCard: function () {
            if (!postCardInfo.canSubmit) {
                return;
            }
            postCardInfo.canSubmit = false;
            var tmpCropBox = $("#cropBox");
            tmpCropBox.addClass("cropBoxCantSubmit").removeClass("cropBoxCanSubmit");
            var tmpPicture = $("#pic");
            tmpPicture.hide();
            postCardInfo.currentDate.cropBox.location.x = tmpCropBox.position().left;
            postCardInfo.currentDate.cropBox.location.y = tmpCropBox.position().top;
            ...
            postCardInfo.currentDate.cropBox.size.height = tmpCropBox.height();
            $.ajax({
                url: "postCard/getNextPictureToCut",
                type: "post",
        ...
        setAllInfo: function (data) {
            ...
            $("#pic").attr("src", tmpData.picturePath).css({
                "width": "auto",
                "height": "auto"
            });
            postCardInfo.init();
