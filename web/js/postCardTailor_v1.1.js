...
function PostCard(obj) {
    ...
    function initPic() {
        ...
        orignalSize.width = $pictureBack.width();//设置原始尺寸的宽度
        orignalSize.height = $pictureBack.height();//设置原始尺寸的高度
    ...
    function init() {
        ...
        //调整一下图片的尺寸
        //如果图片的宽度比较大的话，适应高度
        if (orignalSize.width / orignalSize.height > $cropBox.width() / $cropBox.height()) {//如果原始长宽比 比较 长
            $pictureBack.css({
                width: $cropBox.height() * orignalSize.width / orignalSize.height,
                height: $cropBox.height()
            });
        } else {//如果原始长宽比比较方，或者纵向的话
            $pictureBack.css({
                width: $cropBox.width,
                height: $cropBox.width() * orignalSize.height / orignalSize.width
            });
        }
    ...
    function checkPictureSize() {
        ...
        if ($pictureBack.width() < $cropBox.width() && $pictureBack.height() < $cropBox.height()) {//两边都小于裁切框
            //如果上下一定有白边
            if (orignalSize.width / orignalSize.height > $cropBox.width() / $cropBox.height()) {
                $pictureBack.css(
                    {
                        width: $cropBox.width()
                    }
            ...
            if (orignalSize.width / orignalSize.height < $cropBox.width() / $cropBox.height()) {
                $pictureBack.css({//这一个是正方形
                    height: $cropBox.width() * orignalSize.height / orignalSize.width//高度大于裁切框
                })
            } else {//这一个是长方形
                $pictureBack.css({
                    width: $cropBox.height() * orignalSize.width / orignalSize.height//宽度大于裁切框
                })
            }
        }
